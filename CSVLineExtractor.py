...
def process_row(row, save=False):
    foreground_path = 'Exp_90_92/'+row['Image File']
    background_path = 'Exp_90_92/'+row['Background File']
    ...
    if save:
        black_rect = Image.new('RGB', (right - left, lower - upper), (0, 0, 0))
        crop_composite = Image.new('RGBA', composite_bg.size)

        if foreground.mode != 'RGBA':
            foreground = foreground.convert('RGBA')
        transparency_factor = 0.5
        r, g, b, a = foreground.split()
        a = a.point(lambda i: i * transparency_factor)
        foreground = Image.merge('RGBA', (r, g, b, a))
        mask = foreground.split()[3]

        crop_composite.paste(composite_bg, (0, 0))
        crop_composite.paste(foreground, (x_pos, y_pos), mask=mask)
        crop_composite.paste(black_rect, (left, upper))
        gray_values = np.array(composite_bg.convert('L'))[upper, left:right]
        # print(len(gray_values))
        return gray_values.tolist(), crop_composite, green_mask_thin#, green_mask_thin_ar
    else:
        gray_values = np.array(composite_bg.convert('L'))[upper, left:right]
        return gray_values, None, None#, None
...
def main():
    ...
    save_interval = 1
    output_dir = 'Exp_90_92/Generated overlays'
    output_json = 'Exp_90_92/test_90_92.json'
    ...
    for index, row in df.iterrows():
        ...
        save = (index + 1) % save_interval == 0
        # gray_values, crop_composite, green_mask, ar_mask = process_row(row, save=save)
        gray_values, crop_composite, green_mask = process_row(row, save=save)
    ...
    df['Intensity Values'] = gray_values_list
    df.to_csv(f'{csv_path[:-4]}_augmented.csv', index=False)
    # print(gray_values_list)
    # print(len(gray_values_list[0]))
    # print(gray_values_list[0])

    # Save to file
    with open(output_json, 'w') as f:
        json.dump(gray_values_list, f)

if __name__ == "__main__":
    main()

# df = pd.read_csv('Exp_90_92/test_90_augmented.csv')
