...
# Hybrid global-plus-zone ResNet50 sweep.
#
# Default sweep:
# - 5 folds
# - 3 seeds
# - one global encoder + one shared crop encoder
# - crop branch frozen for first 5 epochs
# - corrected zone masking
#
# Example:
# bash scripts/run_hybrid_global_plus_zone.sh \
...
FOLDS_CSV="0,1,2,3,4"
SEEDS_CSV="0,1,2"
ROOT_OUTPUT_DIR="output_fundus_zone_hybrid"
LOG_DIR="logs/fundus_zone_hybrid"
CSV_PREFIX="fold"
THRESHOLDS_JSON=""
FUNDUS_PRETRAINED_CKPT=""
HYBRID_FREEZE_CROP_EPOCHS=5
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --fundus_pretrained_ckpt)
      ...
      shift 2
      ;;
    --hybrid_freeze_crop_epochs)
      HYBRID_FREEZE_CROP_EPOCHS="$2"
      shift 2
      ;;
    *)
...
COMMON_ARGS=(
  ...
  --input_mode hybrid_global_plus_zone
  --zone_template_json "$ZONE_TEMPLATE_JSON"
  --hybrid_freeze_crop_epochs "$HYBRID_FREEZE_CROP_EPOCHS"
