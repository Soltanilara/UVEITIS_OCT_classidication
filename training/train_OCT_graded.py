...
parser.add_argument('--pickby', type=str, default='qwk', choices=['qwk','f1','auc'], help='Criterion for picking the best model')
...
parser.add_argument('--loss', type=str, default='wasserstein', choices=['CE','focal','wasserstein'], help='Loss function (default: wasserstein)')
...
parser.add_argument('--output_path', type=str, default='output_graded_split_1', help='Path to save the output')
...
def load_data(csv_file, csvpath, folder):
    ...
    label_mapping = {'negative': 0, 'mild': 1, 'moderate': 2, 'severe': 3}
    if args.mimo:
        label_mapping = {'negative': 0, 'mild': 1, 'moderate': 1, 'severe': 2}
    elif args.mose:
        label_mapping = {'negative': 0, 'mild': 1, 'moderate': 2, 'severe': 2}
    df['Label'] = df['Label'].map(label_mapping)
    # df['Label'] = df['Label'].map(lambda x: 0 if x == 'negative' else 1 if x == 'mild' else 2 if x == 'moderate' else 3)
...
classWeights = list(np.load(args.csvpath +'/gradedWeights.npy').astype(np.float32))
if args.loss == 'CE':
    if args.unweighted:
...
def train_model(model, criterion, optimizer, num_epochs=args.num_epochs, ckpt_interval = args.ckpt_interval):
    ...
    stats = {'train_loss':[], 'train_acc':[], 'train_f1':[], 'train_qwk':[], 'val_loss':[], 'val_acc':[], 'val_f1':[], 'val_qwk':[], 'val_auc':[]}
    for epoch in range(num_epochs):
        ...
        stats['train_qwk'].append(train_f1)

        print(f'Epoch {epoch}/{num_epochs - 1}, Loss: {epoch_loss:.4f}, Acc: {epoch_acc:.4f}, '
              f'F1: {train_f1:.4f}, qwk: {train_qwk:.4f}')
        ...
        stats['val_qwk'].append(val_qwk)

        # Print the results
        print(f'Validation Loss: {val_loss:.4f}, Validation Acc: {val_acc: .4f}, '
              f'Validation F1: {val_f1:.4f}, Validation qwk: {val_qwk:.4f}, Validation AUC: {val_auc:.4f}')

        # Call early stopping
        if args.pickby=='auc':
            early_stopping(val_auc, model)
        elif args.pickby=='qwk':
            early_stopping(val_qwk, model)
        if args.pickby=='f1':
...
def evaluate_model(model, test_loader):
    ...
    with open(filename, 'a', newline='') as csvfile:
        ...
        csvwriter.writerow([acc, precision, recall, f1, qwk])
    ...
    if args.mimo:
        df_probs = pd.DataFrame(all_probs, columns=['Prob_Negative', 'Prob_Mild_Moderate', 'Prob_Severe'])
        df_probs.insert(3, 'True_Label', all_labels)
    elif args.mose:
        df_probs = pd.DataFrame(all_probs, columns=['Prob_Negative', 'Prob_Mild', 'Prob_Moderate_Severe'])
        df_probs.insert(3, 'True_Label', all_labels)
    else:
        df_probs = pd.DataFrame(all_probs, columns=['Prob_Negative', 'Prob_Mild', 'Prob_Moderate', 'Prob_Severe'])
        df_probs.insert(4, 'True_Label', all_labels)
...
if args.mode == 'train':
    ...
    plt.subplot(1, 2, 1)
    plt.plot(epochs, history['train_qwk'], label='Training QWK Score')
