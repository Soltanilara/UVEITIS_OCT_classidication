...
parser.add_argument('--beta', type=float, default=0, help='Orthogonalization intensity for complement tuning (default: 0)')
parser.add_argument('--epsilon', type=float, default=0, help='Correlation coefficient denominator constant (default 1e-6)')
...
parser.add_argument('--output_path', type=str, default='./output_spearmancorrloss (split_1), Uv only', help='Path to save the output')
...
folder_name = f"{args.protocol}_{args.model}_autoencoder_{f'pretraining_{args.pretraining}_' if args.protocol!='scratch' else ''}batch_{args.batch_size}_{args.loss}_lr_{args.lr}{f'_minreduce' if args.minreduce else ''}_epochs_{args.num_epochs}_eps_{args.epsilon}{f'_earlystop_patience_{args.patience}' if args.earlystop else ''}{f'_beta_{args.beta}' if args.protocol=='AE' else ''}_seed_{args.seed}"
...
def load_data(csv_file, csvpath, folder):
    ...
    # labels = torch.tensor(np.vstack(labels)/255, dtype=torch.float).unsqueeze(1).unsqueeze(2)
    # labels = F.interpolate(labels,size=(1,512),mode='bilinear', align_corners=False)
    # labels = labels.squeeze(2).squeeze(1)
    if args.minreduce:
...
train_dataset = CustomImageDataset('trainUv.csv', args.csvpath, args.dataset_path, transform=transform)
...
# Test data
test_dataset = CustomImageDataset('testUv.csv', args.csvpath, args.dataset_path, transform=transform)
...
# Validation data
val_dataset = CustomImageDataset('valUv.csv', args.csvpath, args.dataset_path, transform=transform)
...
if args.protocol == 'AE':
    ...
    class Regressor(nn.Module):
        def __init__(self):
            ...
            self.conv5 = nn.Conv2d(3, 1, kernel_size=(512, 3), padding=(0,1))
            ...
            # self.conv6 = nn.Conv2d(3, 1, kernel_size=(512,1))
            # self.fc = nn.Linear(262144, 512)
    ...
    class Autoencoder(nn.Module):
        ...
        def forward(self, x):
            ...
            decoded = torch.sigmoid(self.decoder(x))
    ...
    if args.model not in {'resnet50w5'}:
        num_ftrs = model.fc.in_features
        model.fc = nn.Linear(num_ftrs, 512)  # Adjusting for CIFAR-10 classes
...
if args.loss == 'ssim':
    ...
    class SpearmanCorrelationLoss(nn.Module):
        ...
        def forward(self, y_pred, y_true, epsilon = args.epsilon):
            rank_true = torch.argsort(torch.argsort(y_true, dim=1), dim=1).float()
            rank_pred = torch.argsort(torch.argsort(y_pred, dim=1), dim=1).float()
            mean_true = torch.mean(rank_true, dim=1, keepdim=True)
            mean_pred = torch.mean(rank_pred, dim=1, keepdim=True)
            numerator = torch.sum((rank_true - mean_true) * (rank_pred - mean_pred), dim=1)
            std_true = torch.std(rank_true, dim=1, unbiased=False)
            std_pred = torch.std(rank_pred, dim=1, unbiased=False)
            denominator = std_true * std_pred * rank_true.size(1)
            correlation = numerator / (denominator + epsilon)
            avg_correlation = torch.mean(correlation)
            loss = 1 - avg_correlation
            return loss
    def soft_rank(x, epsilon=1e-5):
        pairwise_diffs = x.unsqueeze(2) - x.unsqueeze(1)
        soft_sign = torch.sigmoid(pairwise_diffs / epsilon)
        soft_rank = torch.sum(soft_sign, dim=2)
        return soft_rank
    class SoftSpearmanCorrelationLoss(nn.Module):
        def __init__(self):
            super(SoftSpearmanCorrelationLoss, self).__init__()
        def forward(self, y_pred, y_true, epsilon = args.epsilon):
            rank_true = soft_rank(y_true)
            rank_pred = soft_rank(y_pred)
            mean_true = torch.mean(rank_true, dim=1, keepdim=True)
            mean_pred = torch.mean(rank_pred, dim=1, keepdim=True)
            numerator = torch.sum((rank_true - mean_true) * (rank_pred - mean_pred), dim=1)
            std_true = torch.std(rank_true, dim=1, unbiased=False)
            ...
            loss = 1 - avg_correlation
            return loss
    criterion = SoftSpearmanCorrelationLoss()
    evalCriterion = SpearmanCorrelationLoss()
        ...
        with torch.no_grad():
            for inputs, labels in val_loader:
                ...
                if args.protocol == 'complement':
                    ...
                    decoded, outputs = model(inputs)
                    loss_regression = evalCriterion(outputs, labels)
                    loss = loss_regression + args.beta*criterion(decoded, torch.sigmoid(inputs))
...
def evaluate_model(model, test_loader):
    model.eval()
    running_ssim = 0
    true_images = np.zeros((len(test_loader.dataset),512))
    pred_images = np.zeros((len(test_loader.dataset),512))
    if args.protocol == 'AE':
    ...
    with torch.no_grad():
        for i,(inputs, labels) in enumerate(eval_progress_bar):
            ...
            if args.loss == 'ssim':
                ...
                running_ssim += inputs.size(0) * (1-evalCriterion(outputs,labels,epsilon=1e-6).item())
    ...
    corrfile = eval_output_path + "/sample_correlations.csv"
    ...
    for i, index in enumerate(vis_indices):
        ...
        true_im_tensor = torch.tensor(true_images[index].reshape(1,-1))
        pred_im_tensor = torch.tensor(pred_images[index].reshape(1,-1))
        correlation = 1-evalCriterion(pred_im_tensor,true_im_tensor,epsilon=1e-6)

        # Write metrics to CSV
        with open(corrfile, 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow([correlation])
        #
        ...
        # axes[i, 1].set_title(f'Resized (Size: 512)')
        axes[i, 1].axis('off')
