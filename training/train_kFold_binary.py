...
from models.retfound_zone_model import RetFoundDinoZoneModel
...
NUM_ZONES = 10
ZONE_AWARE_NUM_ZONES = 9
NUM_CLASSES = 2
ORIGINAL_NUM_CLASSES = 3
ZONE_COLUMNS = [f"Zone{i}_label" for i in range(1, NUM_ZONES + 1)]
FALLBACK_EXTS = [".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp"]
ZONE_AWARE_INPUT_MODE = "retfound_dino_zone"
ZONE_AWARE_MODEL_NAMES = {"zone_aware_retfound", "retfound_dino_zone"}
...
parser.add_argument(
    ...
    choices=[
        "full_image_zone_head",
        "zone_crops_shared",
        "hybrid_global_plus_zone",
        "zone_masked_shared",
        ZONE_AWARE_INPUT_MODE,
    ],
)
...
if args.model in ZONE_AWARE_MODEL_NAMES:
    args.input_mode = ZONE_AWARE_INPUT_MODE


def is_zone_aware_retfound() -> bool:
    return args.input_mode == ZONE_AWARE_INPUT_MODE


if is_zone_aware_retfound() and args.pretraining != "retfound_dinov2":
    print("Using --pretraining retfound_dinov2 for Zone-Aware RETFound-DINO.")
    args.pretraining = "retfound_dinov2"

if is_zone_aware_retfound() and args.drop_missing_zone_rows != "none":
    print("Using --drop_missing_zone_rows none for Zone-Aware RETFound-DINO; provide a mask-ready CSV.")
    args.drop_missing_zone_rows = "none"


ACTIVE_NUM_ZONES = ZONE_AWARE_NUM_ZONES if is_zone_aware_retfound() else NUM_ZONES
ACTIVE_NUM_CLASSES = 1 if is_zone_aware_retfound() else NUM_CLASSES
...
if is_zone_aware_retfound() and args.protocol == "complement":
    raise ValueError("Zone-Aware RETFound-DINO does not support protocol='complement'.")

if is_zone_aware_retfound() and (args.hflip or args.elastic):
    raise ValueError("Zone-Aware RETFound-DINO requires masks aligned to the image; disable --hflip and --elastic.")

if is_zone_aware_retfound() and args.mixup:
    raise ValueError("Zone-Aware RETFound-DINO does not support mixup because anatomical masks cannot be mixed safely.")
...
def build_folder_name() -> str:
    bits = [
        ...
        "bce" if is_zone_aware_retfound() else args.loss.lower(),
        ...
        f"zones{ACTIVE_NUM_ZONES}x{ACTIVE_NUM_CLASSES}",
        f"seed_{args.seed}",
    ]
...
def zone_stack_from_mask_array(mask: np.ndarray, mask_path: str) -> np.ndarray:
    """Return binary anatomical masks with shape [10,H,W].

    Supports both legacy 2D label maps (`1..10`) and canonical 3D mask stacks
    (`[10,H,W]`) generated by batch_slice_fa_apply_fp.py.
    """
    if mask.ndim == 2:
        return np.stack([(mask == zone_id) for zone_id in range(1, NUM_ZONES + 1)], axis=0).astype(np.uint8)

    if mask.ndim == 3:
        if mask.shape[0] >= NUM_ZONES:
            return (mask[:NUM_ZONES] > 0).astype(np.uint8)
        if mask.shape[-1] >= NUM_ZONES:
            return np.moveaxis((mask[..., :NUM_ZONES] > 0).astype(np.uint8), -1, 0)

    raise ValueError(f"Unsupported zone mask shape in {mask_path}: {tuple(mask.shape)}")


def label_map_from_zone_stack(zone_stack: np.ndarray) -> np.ndarray:
    label_map = np.zeros(zone_stack.shape[1:], dtype=np.uint8)
    for zone_idx in range(zone_stack.shape[0]):
        label_map[zone_stack[zone_idx].astype(bool)] = zone_idx + 1
    return label_map


def resize_zone_stack(zone_stack: np.ndarray, size: tuple[int, int]) -> np.ndarray:
    """Nearest-neighbor resize a [Z,H,W] mask stack to PIL size (width,height)."""
    resized = [
        np.array(
            Image.fromarray(zone_stack[z].astype(np.uint8), mode="L").resize(size, Image.Resampling.NEAREST),
            dtype=np.uint8,
        )
        for z in range(zone_stack.shape[0])
    ]
...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_mask = None
        zone_stack = None
        if args.apply_mask or self.input_mode in {"zone_masked_shared", ZONE_AWARE_INPUT_MODE}:
            ...
            if zone_stack.shape[1:] != (image.height, image.width):
                zone_stack = resize_zone_stack(zone_stack, image.size)
            label_mask = label_map_from_zone_stack(zone_stack)
        if args.apply_mask:
            image_arr = np.array(image, dtype=np.uint8)
            image = Image.fromarray(image_arr * (label_mask > 0)[..., None].astype(np.uint8), mode="RGB")
        if self.input_mode == ZONE_AWARE_INPUT_MODE:
            if zone_stack is None:
                raise ValueError("Zone-Aware RETFound-DINO requires anatomical masks.")
            image_arr = np.array(image, dtype=np.uint8)
        ...
        if self.input_mode in {"full_image_zone_head", "hybrid_global_plus_zone", ZONE_AWARE_INPUT_MODE}:
            ...
            full_image_tensor = self.full_image_transform(image.copy())
            sample["full_image"] = full_image_tensor
            if self.input_mode == ZONE_AWARE_INPUT_MODE:
                mask_size = (int(full_image_tensor.shape[-1]), int(full_image_tensor.shape[-2]))
                zone_masks = resize_zone_stack(zone_stack[:ZONE_AWARE_NUM_ZONES], mask_size)
                sample["zone_masks"] = torch.from_numpy(zone_masks.astype(np.float32))
        ...
        if self.input_mode == "zone_masked_shared":
            ...
            zone_masked_images = extract_zone_masked_images(image, label_mask)
...
def compute_zone_pos_weights(labels: torch.Tensor, observed_mask: torch.Tensor, num_zones: int) -> torch.Tensor:
    pos_weights = torch.ones(num_zones, dtype=torch.float32)
    for z in range(num_zones):
        zone_mask = observed_mask[:, z]
        if not zone_mask.any():
            continue
        ...
        if positives > 0.0 and negatives > 0.0:
            pos_weights[z] = negatives / positives
    return pos_weights
...
def build_model() -> nn.Module:
    ...
    if args.input_mode == ZONE_AWARE_INPUT_MODE:
        if args.pretraining != "retfound_dinov2":
            raise ValueError("retfound_dino_zone requires --pretraining retfound_dinov2.")
        backbone = load_retfound_dinov2_backbone()
        model = RetFoundDinoZoneModel(backbone=backbone, num_zones=ZONE_AWARE_NUM_ZONES)
        maybe_freeze_encoder(model.token_backbone)
        return model
...
class MultiZoneBCECriterion:
    """Masked BCEWithLogits loss for independent zone logits with shape [B,9]."""

    def __init__(self, pos_weight: torch.Tensor | None = None):
        self.pos_weight = pos_weight

    def __call__(self, logits: torch.Tensor, targets: torch.Tensor, observed_mask: torch.Tensor) -> tuple[torch.Tensor, int]:
        num_zones = logits.shape[1]
        targets = targets[:, :num_zones].float()
        observed_mask = observed_mask[:, :num_zones]
        total_loss = logits.sum() * 0.0
        total_observed = 0
        ...
        for z in range(num_zones):
            ...
            if self.pos_weight is not None:
                kwargs["pos_weight"] = self.pos_weight[z].view(1)
            zone_loss = F.binary_cross_entropy_with_logits(
                logits[zone_mask, z],
                targets[zone_mask, z],
                reduction="sum",
                **kwargs,
...
def mixup_batch(batch: dict[str, Any], lam: float, index: torch.Tensor) -> dict[str, Any]:
    mixed = {}
    for key, value in batch.items():
        if key in {"labels", "observed_mask", "zone_masks", "image_file", "visit_id", "patient_id", "eye", "visit_date"}:
            mixed[key] = value
...
def load_zone_thresholds(path: str) -> np.ndarray:
    if not path:
        return np.full(ACTIVE_NUM_ZONES, 0.5, dtype=np.float32)
    ...
    if isinstance(payload, dict):
        thresholds = [payload.get(f"Zone{i}", payload.get(f"Zone{i}_label", 0.5)) for i in range(1, ACTIVE_NUM_ZONES + 1)]
    elif isinstance(payload, list):
        thresholds = payload
    else:
        raise ValueError(f"Unsupported threshold payload in {path}")

    if len(thresholds) != ACTIVE_NUM_ZONES:
        raise ValueError(f"Expected {ACTIVE_NUM_ZONES} thresholds in {path}, found {len(thresholds)}")
...
def forward_logits(model: nn.Module, batch: dict[str, Any]):
    ...
    if args.input_mode == ZONE_AWARE_INPUT_MODE:
        outputs = model(full_image=batch["full_image"], zone_masks=batch["zone_masks"])
        if outputs.ndim != 2 or outputs.shape[1] != ZONE_AWARE_NUM_ZONES:
            raise RuntimeError(f"Expected model output shape [B, {ZONE_AWARE_NUM_ZONES}] but got {tuple(outputs.shape)}")
...
def compute_predictions_from_logits(logits: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    if is_zone_aware_retfound():
        positive_probs = torch.sigmoid(logits)
        preds = (positive_probs >= zone_thresholds_tensor).long()
        probs = torch.stack([1.0 - positive_probs, positive_probs], dim=2)
        return preds, probs
...
def run_epoch(model, loader, criterion_fn, optimizer=None):
    ...
    with context:
        for batch in tqdm(loader, leave=False):
            ...
            all_labels.append(labels[:, : preds.shape[1]].detach().cpu().numpy())
            all_probs.append(probs.detach().cpu().numpy())
            all_masks.append(observed_mask[:, : preds.shape[1]].detach().cpu().numpy())
...
if args.weightedSampling:
    if is_zone_aware_retfound():
        sample_weights = compute_sample_weights(
            train_dataset.labels[:, :ZONE_AWARE_NUM_ZONES],
            train_dataset.observed_mask[:, :ZONE_AWARE_NUM_ZONES],
            zone_class_weights_cpu[:ZONE_AWARE_NUM_ZONES],
        )
    else:
...
zone_thresholds_tensor = torch.tensor(zone_thresholds, dtype=torch.float32, device=device).view(1, ACTIVE_NUM_ZONES)
model = build_model().to(device)
init_wandb()

zone_class_weights = None if args.unweighted else zone_class_weights_cpu.to(device)
if is_zone_aware_retfound():
    pos_weight = None if args.unweighted else compute_zone_pos_weights(
        train_dataset.labels,
        train_dataset.observed_mask,
        ZONE_AWARE_NUM_ZONES,
    ).to(device)
    criterion_fn = MultiZoneBCECriterion(pos_weight=pos_weight)
else:
...
def save_train_metadata(best_epoch: int | None, best_val_mean_f1: float | None):
    metadata = {
        ...
        "num_zones": ACTIVE_NUM_ZONES,
        "num_classes": ACTIVE_NUM_CLASSES,
        "zone_aware_retfound_dino": bool(is_zone_aware_retfound()),
        "input_mode": args.input_mode,
