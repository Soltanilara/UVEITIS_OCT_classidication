...
def _validate_zone_values(zone_df: pd.DataFrame, csv_file: str) -> pd.DataFrame:
    ...
    if not np.isclose(zone_df.values, as_int.values).all():
        raise ValueError(f"Non-integer zone class value found in {csv_file}; expected integer classes in {{0,1,2}}.")

    bad_mask = (as_int.values < 0) | (as_int.values >= ORIGINAL_NUM_CLASSES)
    if bad_mask.any():
...
class EarlyStopping:
    ...
    def save_checkpoint(self, metric_value: float, model: nn.Module, epoch: int):
        if self.verbose:
            print(
                f"Validation mean binary F1 improved ({self.best_metric:.6f} -> {metric_value:.6f}). Saving model ..."
            )
...
def compute_zone_metrics(y_true: np.ndarray, y_pred: np.ndarray):
    ...
    summary = {
        ...
        "mean_binary_f1": float(df["BinaryF1"].mean()),
        "mean_precision": float(df["Precision"].mean()),
        "mean_recall": float(df["Recall"].mean()),
        "mean_specificity": float(df["Specificity"].mean()),
        "macro_f1_across_zones": float(df["BinaryF1"].mean()),
        "micro_f1_flat": float(f1_score(flat_true, flat_pred, average="micro", labels=[0, 1], zero_division=0)),
        "macro_f1_flat": float(f1_score(flat_true, flat_pred, average="macro", labels=[0, 1], zero_division=0)),
    }
    return df, summary
...
def run_epoch(model, loader, criterion_fn, optimizer=None):
    ...
    zone_df, summary = compute_zone_metrics(y_true, y_pred)
...
def train_model(model, criterion_fn, optimizer, num_epochs=args.num_epochs, ckpt_interval=args.ckpt_interval):
    ...
    for epoch in range(num_epochs):
        ...
        print(
            ...
            f"MeanBinaryF1: {train_out['mean_f1']:.4f}"
        )
        ...
        if not args.final:
            ...
            print(
                ...
                f"MeanBinaryF1: {val_out['mean_f1']:.4f}"
            )
...
def evaluate_on_split(model, loader, criterion_fn, save_path: str, split_name: str):
    ...
    summary = {
        ...
        "mean_binary_f1": summary["mean_binary_f1"],
        "mean_precision": summary["mean_precision"],
        "mean_recall": summary["mean_recall"],
        "mean_specificity": summary["mean_specificity"],
        "micro_f1_flat": summary["micro_f1_flat"],
        "macro_f1_flat": summary["macro_f1_flat"],
        "macro_f1_across_zones": summary["macro_f1_across_zones"],
    }
    ...
    print(
        ...
        f"MeanBinaryF1={summary['mean_binary_f1']:.4f}. Saved metrics to {save_path}"
    )
...
if args.mode == "train":
    if args.final:
        ...
        metadata = {
            ...
            "best_val_mean_binary_f1": float(best_val_mean_f1),
            "num_zones": NUM_ZONES,
