...
parser.add_argument('--harmonic', action='store_true', help='Enable harmonic classifier')
parser.add_argument('--n_harmax', type=int, default=45, help='HarMax n parameter')
...
parser.add_argument('--output_path', type=str, default='output_split_1', help='Path to save the output')
...
folder_name = (f"{args.protocol}_{args.model}_{f'pretraining_{args.pretraining}_' if args.protocol!='scratch' else ''}"
               ...
               f"{f'harmonic_{args.n_harmax}_' if args.harmonic else ''}"
...
class HarmonicClassifier(nn.Module):
    def __init__(self, in_features, out_features, n=args.n_harmax, device=None, dtype=None):
        factory_kwargs = {'device': device, 'dtype': dtype}  # Ensure compatibility
        super(HarmonicClassifier, self).__init__()
    ...
    def forward(self, x):
        x = x.contiguous()
        # Compute Euclidean distance between each input vector and each weight vector
        distances = torch.cdist(x.unsqueeze(1), self.weight.unsqueeze(0), p=2).squeeze(1)  # (batch_size, num_classes)
        logits = -self.n*torch.log(distances+torch.finfo(torch.float32).eps)
        return logits
...
if args.protocol == 'complement':
    ...
    if args.model in {'densenet121','densenet201'}:
        ...
        if args.harmonic:
            model.classifier = HarmonicClassifier(num_ftrs, 2)  # Adjusting for CIFAR-10 classes
        else:
            model.classifier = nn.Linear(num_ftrs, 2)  # Adjusting for CIFAR-10 classes
    else:
        num_ftrs = model.fc.in_features
        if args.harmonic:
            model.fc = HarmonicClassifier(num_ftrs, 2)  # Adjusting for CIFAR-10 classes
        else:
    ...
    if args.pretraining == 'swav':
        if args.model=='resnet50':
            ...
            model = nn.Sequential(
                ...
                nn.Linear(10240,2) if not args.harmonic else HarmonicClassifier(10240, 2)
            )
        ...
        if args.model=='resnet18':
            ...
            if args.harmonic:
                model.classifier[6]=HarmonicClassifier(4096,2)
            else:
    ...
    if args.model not in {'resnet50w5','vitb14_dino','vgg19_bn','densenet121','densenet201'}:
        num_ftrs = model.fc.in_features
        if args.harmonic:
            model.fc = HarmonicClassifier(num_ftrs, 2)
        else:
            model.fc = nn.Linear(num_ftrs, 2)
    elif args.model == 'vitb14_dino':
        num_ftrs = model.linear_head.in_features
        if args.harmonic:
            model.linear_head = HarmonicClassifier(num_ftrs, 2)
        else:
