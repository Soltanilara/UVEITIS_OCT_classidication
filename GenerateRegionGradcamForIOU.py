...
SCORE_DIR     = "output_ext_test/final_finetune_resnet50_pretraining_swav_CE_loss_unweighted_batch_64_lr_0.001_decay_1e-05_10_10_1_1e-05_epochs_100_hflip_seed_0/GradCAM_Positive_Scores"           # folder that holds 0.npy, 1.npy, …
OUT_DIR       = "Exp_90_92/GradCAM"
...
for group_key, idx_list in groups.items():
    if group_key in {'Patient090/20250107/OS/FO/', 'Patient092/20250402/OS/FO/'}:
        idx_list.sort()                              # ensure row order
        n_imgs        = len(idx_list)
        rows_per_img  = 1               # integer division
        processed_rows = []

        for i in idx_list:
            f_score = Path(SCORE_DIR) / f"{i}_raw.npy"
        ...
        # patient_map = np.repeat(patient_map, 32, axis=1)
        ...
        save_name = group_key.rstrip("/").replace("/", "_") + ".npy"
        np.save(out_path / save_name, patient_img)
