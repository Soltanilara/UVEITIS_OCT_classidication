...
X = data.iloc[:, :-2].values  # Feature vectors (first 2048 columns)
y_true = data['True_Label'].values  # True labels
...
fig2 = px.scatter_3d(df_mds, x='Dim1', y='Dim2', z='Dim3',
                    ...
                    color_continuous_scale='RdBu_r',  # Use the Reds color scale again for consistency
...
fig2.update_traces(marker=dict(size=3))  # Reduce marker size

# Save the second plot to the same directory as the CSV file
plot_file2 = os.path.join(csv_path, f"3D_visualization_{method}_true_classes.html")
