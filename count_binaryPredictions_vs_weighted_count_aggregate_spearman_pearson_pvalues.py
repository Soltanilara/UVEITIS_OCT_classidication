...
def aggregate_correlations_and_pvalues(
    ...
    for (patient, eye, region), subdf_group in grouped:
        ...
        for date_val, date_data in sub_grouped_by_date:
            ...
            norm_pos = date_data["pred_pos_label"].mean()
...
def compute_per_patient_date_aggregates(
    ...
    for (patient, date_val, eye, region), group_data in grouped:
        ...
        normalized_positive = group_data["pred_pos_label"].mean()
...
if __name__ == "__main__":
    # 1) Load the CSV
    # severity_csv = "CombinedExtNebTests.csv"  # ← ground-truth, same format as before
    # pred_csv = "CombinedExtNebTests_Probabilities.csv"  # ← Prob_Positive, Prob_Negative, True_Label
    ...
    # out_folder = "Count (test hard labels) ext test"
    ...
    out_folder = "Count (test hard labels) nebraska"
    ...
    THRESH = 0.085  # ← set to whatever cut-off you need
    df_truth["prob_positive"] = df_pred["Prob_Positive"].values
    df_truth["pred_pos_label"] = (df_truth["prob_positive"] >= THRESH).astype(int)
