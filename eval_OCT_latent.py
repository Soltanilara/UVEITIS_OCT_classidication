...
parser.add_argument('--dataset_path', type=str, default='Dataset 09192024', help='Path to images')
...
def evaluate_model(model, test_loader):
    # Global variable to store the latent features
    # latent_features = []

    # Hook function to capture the output of the avgpool layer
    ...
    with torch.no_grad():
        for inputs, labels in eval_progress_bar:
            ...
            if args.protocol == 'complement':
                ...
                outputs = model(inputs)
                all_latents.extend(latent_features.view(latent_features.size(0), -1).cpu().numpy())
            _, preds = torch.max(outputs, 1)
    ...
    # Save latent features, ground truth labels, and predicted probabilities (class 1) to a CSV
    df_latents = pd.DataFrame(all_latents)
    df_latents['True_Label'] = all_labels
    df_latents['Pred_Prob_Class1'] = [p[1] for p in all_probs]  # Probabilities for class 1
    df_latents.to_csv(os.path.join(eval_output_path, 'test_features.csv'), index=False)
