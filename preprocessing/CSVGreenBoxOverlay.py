...
def process_row(row, save=False):
    ...
    if save:
        ...
        rect_left = left_line
        rect_top = top_line
        rect_right = right_line  # inclusive
        rect_bottom = bottom_line  # inclusive

        # ------------------------------------------------------------------
        # 3) Create a transparent image that contains ONLY the green outline
        # ------------------------------------------------------------------
        outline_w = 1  # thickness in pixels – adjust if you like

        rect_w = rect_right - rect_left + 1
        rect_h = rect_bottom - rect_top + 1
        # print(rect_w,rect_h)

        green_rect = Image.new("RGBA", (rect_w, rect_h), (0, 0, 0, 0))
        draw = ImageDraw.Draw(green_rect)
        draw.rectangle(
            [0, 0, rect_w - 1, rect_h - 1],
            outline=(0, 255, 0, 255),  # pure green RGBA
            width=outline_w
        )

        # ------------------------------------------------------------------
        # 4) Paste that outline onto crop_composite
        #    (the alpha channel is used as the mask → only the outline shows)
        # ------------------------------------------------------------------
        crop_composite.paste(green_rect, (left, rect_top), mask=green_rect)
...
def main():
    ...
    # save_interval = 45
    output_dir = 'Generated overlays (ext_test, test)'
    os.makedirs(output_dir, exist_ok=True)

    # gray_values_list = []
    visit = set()
    for index, row in df.iterrows():
        if '/'.join(row['Image File'].split('/')[:-1]) not in visit and row['Image File'][-8:-4] not in {'0000','0048'}:
            # if index<18281:
