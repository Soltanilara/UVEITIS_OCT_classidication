...
old_to_new_df = pd.read_csv('OldToNewIDs.csv')
old_to_new_dict = dict(zip(old_to_new_df["Old Patient ID"],
                           old_to_new_df["New Patient ID"]))
...
# ------------------------------------------------------------------------------
# CHANGED SECTION: Instead of randomly sampling val/test, load them
# from the previously generated CSVs in Dataset 09192024/split_{seedval}/
# ------------------------------------------------------------------------------
# Load previously generated val and test CSVs
val_old = pd.read_csv(f'Results for Dataset 09192024/split_{seedval}/val.csv')
test_old = pd.read_csv(f'Results for Dataset 09192024/split_{seedval}/test.csv')

# Extract prefixes from these old files
val_old['Prefix'] = val_old['Image File'].apply(lambda x: x.split('/')[0])
test_old['Prefix'] = test_old['Image File'].apply(lambda x: x.split('/')[0])

# Use the dictionary to map old prefixes to new ones
val_old['NewPrefix'] = val_old['Prefix'].map(old_to_new_dict)
test_old['NewPrefix'] = test_old['Prefix'].map(old_to_new_dict)

# Extract the unique new prefixes
val_prefixes = val_old['NewPrefix'].unique()
test_prefixes = test_old['NewPrefix'].unique()
# ------------------------------------------------------------------------------
